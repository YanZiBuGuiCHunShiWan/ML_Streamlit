...
def paper_personalization():
    st.markdown("## :blue[大模型个性化]")
    st.markdown("### :blue[[When Large Language Models Meet Personalization: Perspectives of Challenges and Opportunities]()]")

def paper_datastes():
    st.markdown("## :blue[数据集]")
    ...
    st.markdown("&emsp;&emsp;首先预训练一个GPT2模型，将人类标注的strategy和Roberta标注的strategy混合，然后划分训练集、验证集、测试集。通过$prepending$即扩展其词汇表添加自定义的特殊令牌将不同的句子拼接起来，具体如下：\
    ...
    st.markdown("&emsp;&emsp;表明在预训练时加上句子的strategy标签能够显著提升模型的语义理解能力。")
    
    
def paper_model_training():
    st.markdown("## :blue[大模型训练]")
    st.markdown("### :blue[[Baichuan 2: Open Large-scale Language Models](https://cdn.baichuan-ai.com/paper/Baichuan2-technical-report.pdf)]")
    
    pass
pages={
    "大模型个性化": paper_personalization,
    "数据集相关": paper_datastes,
    "模型训练": paper_model_training
}
# 添加侧边栏菜单
selection = st.sidebar.radio("论文分类", list(pages.keys()))
